...
(function (root, factory) {
    ...
    dayPrice = function(){
        ...
        $('.calendar_item').each(function (index, value) { 
            ...
            let month = $(this).attr('data-month') - 1
            let day = $(this).attr('data-day')
            let id = new Date(year,month,day,hours,minutes,seconds,milliseconds).getTime()
            let id2 = new Date(year,month,day,hours,0,0,0).getTime()
          
            setTimeout(() => {
                $(`.lightpick__day[data-time="${id}"]`).append(`<span>${$(this).text()}</span>`)
                $(`.lightpick__day[data-time="${id2}"]`).append(`<span>${$(this).text()}</span>`)
                $(`.lightpick__day[data-time="${id - 1}"]`).append(`<span>${$(this).text()}</span>`)
            }, 100);
        });
    },
